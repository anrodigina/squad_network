...
sl_size = 600
for i in range(0, int(len(cont_features)/sl_size)):
    model.fit(x={'in_qst':embedding[qst_data[i*sl_size:(i+1)*sl_size]],
                 'in_txt':embedding[context_data[i*sl_size:(i+1)*sl_size]],
                 'in_features':cont_features[i*sl_size:(i+1)*sl_size],
                 'in_tags':voc_con[con_tag[i*sl_size:(i+1)*sl_size]],
                 'in_ents':voc_ent[ent_tag[i*sl_size:(i+1)*sl_size]]
                },
              y={'ans_beg':ans_beg[i*sl_size:(i+1)*sl_size], 'ans_end':ans_end[i*sl_size:(i+1)*sl_size]},
              batch_size=200, epochs=1, shuffle=True)
    print(i, 'out of', int(len(cont_features)/sl_size) - 1)
...
model.save_weights('train-v-1.h5')
